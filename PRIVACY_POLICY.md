...
- When the configured operator sends `!guildrequest <Guild ID>` to Hana in a direct message, Hana processes that Guild ID to check whether it is installed there and may return an existing invite or create a new invite only where Discord permissions permit. This request is not stored as a new user profile or conversation-memory record.
...
- When you explicitly request a photo of Hana's current verified travel status or use `/travelphoto`, Hana may request a fresh stock image from Pexels using only a fixed Japan location query that matches that status. Hana attaches a result only when its supplied description matches the approved Japan place; otherwise no image is attached. Hana does not send your Discord message, identity, or location to Pexels, and does not cache these photo results.
