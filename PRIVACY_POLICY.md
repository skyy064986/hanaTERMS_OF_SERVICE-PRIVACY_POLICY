...
- When the configured operator sends `!guildrequest <Guild ID>` to Hana in a direct message, Hana processes that Guild ID to check whether it is installed there and may return an existing invite or create a new invite only where Discord permissions permit. For a reported outage, security concern, misuse, or support request, the operator may use such an available invite to join and investigate the server. The operator can access only channels allowed by Discord permissions and must use the access for that operational purpose; it is not automatic access to the server. This request is not stored as a new user profile or conversation-memory record.
